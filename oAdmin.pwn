...
/////////Colors2/////////////////
//////////Don't Teach This till u understand what is it.
	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 1: //Give Weapons
			{
					...
					GivePlayerWeapon(clickedplayerid,8,1);      //Katana
					GivePlayerWeapon(clickedplayerid,31,99999); //M4
					GivePlayerWeapon(clickedplayerid,24,99999); //Deagle
					GivePlayerWeapon(clickedplayerid,26,99999); //Sawn-Off
					GivePlayerWeapon(clickedplayerid,28,99999); //Micro-SMG
					GivePlayerWeapon(clickedplayerid,34,99999); //Sniper
...
CMD:givepack(playerid, params[])
{
	...
	GivePlayerWeapon(target,8,1);      //Katana
	GivePlayerWeapon(target,31,99999); //M4
	GivePlayerWeapon(target,24,99999); //Deagle
	GivePlayerWeapon(target,26,99999); //Sawn-Off
	GivePlayerWeapon(target,28,99999); //Micro-SMG
	GivePlayerWeapon(target,34,99999); //Sniper
...
CMD:rgun(playerid, params[])
{
    ...
    for(new i = 0; i < MAX_PLAYERS; i ++)
    {
        if(IsPlayerInRangeOfPoint(i, range, x, y, z))
        {
            ...
            GivePlayerWeapon(i,8,1);      //Katana
            GivePlayerWeapon(i,31,99999); //M4
            GivePlayerWeapon(i,24,99999); //Deagle
            GivePlayerWeapon(i,26,99999); //Sawn-Off
            GivePlayerWeapon(i,28,99999); //Micro-SMG
            GivePlayerWeapon(i,34,99999); //Sniper
            new str[200];
...
{
	new str[500],target,Time,reason[128];
	if(pInfo[playerid][AdminLevel] < 3) return SCM(playerid,red,"Error:You Can't Use This Command.");
	if (sscanf(params, "uis",target,Time,reason)) return SCM(playerid,red,"USAGE:jail <PlayerID> <TimeSecs> <Reason>");
    ...
    format(str,sizeof(str),"You Have Jailed For %d Secs By Admin : %s[%d] [Reason: %s]",Time,GetName(playerid),playerid,reason);
    SCM(target,Green,str);
	format(str,sizeof(str),"%s Has Jailed For %d Seconds By Admin : %s[%d] [Reason: %s]",GetName(target),Time,GetName(playerid),playerid,reason);
	SCMTA(Green,str);
	Log("Mute+Jail",str);
	/*format(str,sizeof(str),"~g~ Jail Time : %d",);
    GameTextForPlayer(target, string, 3000, 3);*/
	SetTimer("jail",Time*1000,false);
	format(str,sizeof(str), "4,11Adminintstrator %s[%d] Has Jailed %s[%d] (%d Seconds) | Reason : %s", GetName(playerid),playerid,GetName(target),target,Time,reason);
	IRC_GroupSay(groupID, IRC_CHANNEL, str);
	IRC_GroupSay(GAdmins,IRC_AdminChannel, str);
	return 1;
}
...
CMD:mute(playerid,params[])
{
	new str[500],target,Time,reason[128];
	if(pInfo[playerid][AdminLevel] < 2) return SCM(playerid,red,"Error:You Can't Use This Command.");
	if (sscanf(params, "uis",target,Time,reason)) return SCM(playerid,red,"USAGE:Mute <PlayerID> <TimeSecs> <Reason>");
	if (Mute[target] == 1) return SCM(playerid,red,"Player Already Muted");
	Mute[target] = 1;
    format(str,sizeof(str),"You Have Muted For %d Secs By Admin : %s[%d] [Reason: %s]",Time,GetName(playerid),playerid,reason);
    SCM(target,Green,str);
	format(str,sizeof(str),"%s Has Muted For %d Seconds By Admin : %s[%d] [Reason: %s]",GetName(target),Time,GetName(playerid),playerid,reason);
	SCMTA(Green,str);
	Log("Mute+Jail",str);
	SetTimer("mute",Time*1000,false);
	format(str,sizeof(str), "4,11Adminintstrator %s[%d] Has Muted %s[%d] (%d Seconds) | Reason : %s", GetName(playerid),playerid,GetName(target),target,Time,reason);
	IRC_GroupSay(groupID, IRC_CHANNEL, str);
	IRC_GroupSay(GAdmins,IRC_AdminChannel, str);
	return 1;
}
...
CMD:eventblock(playerid,params[])
{
	new reason[30],target,str[111];
    if(pInfo[playerid][AdminLevel] < 4) return SCM(playerid,red,"Error:You Can't Use This Command.");
    if(EventInfo[Current]==0) return SCM(playerid,red,"Error: There is no events for now.");
    if(sscanf(params, "us[30]", target,reason)) return SCM(playerid, red, "USAGE: /eventblock <PlayerID> <Reason>");
    ...
    SCM(target,red,str);
    format(str,sizeof(str),"Admin:{452820}%s Blocked Player %s From This Event | Reason: %s",GetName(playerid),GetName(target),reason);
    AdminChat(-1,str);
    return 1;
}
...
CMD:endevent(playerid,params[])
{
	...
	new str[122];
	format(str,sizeof(str),"Event Has End | The Winner: %s.",winner);
...
IRCCMD:jail(botid, channel[], user[], host[], params[])
{
    if(IRC_IsHalfop(botid, channel, user))
	{
	    new str[500],id,Time,reason[128];
	    if (sscanf(params, "uis",id,Time,reason)) return IRC_GroupSay(groupID, channel, "4USAGE:jail <PlayerID> <TimeSecs> <Reason>");
	    ...
	    format(str,sizeof(str),"You Have Jailed For %d Secs By Admin : %s From IRC [Reason: %s]",Time,user,reason);
    	SCM(id,Green,str);
		format(str,sizeof(str),"%s(%d) Has Jailed For %d Seconds By Admin : %s From IRC [Reason: %s]",GetName(id),id,Time,user,reason);
		SCMTA(Green,str);
		SetTimer("jail",Time*1000,false);\
		format(str,sizeof(str), "4,11Adminintstrator From IRC: %s Has Jailed %s[%d] (%d Seconds) | Reason : %s",user,GetName(id),id,Time,reason);
		IRC_GroupSay(groupID, IRC_CHANNEL, str);
		IRC_GroupSay(GAdmins,IRC_AdminChannel, str);
		return 1;
	}
...
IRCCMD:mute(botid, channel[], user[], host[], params[])
{
	...
	{
        new str[500],id,Time,reason[128];
        if (sscanf(params, "uis",id,Time,reason)) return IRC_GroupSay(groupID, channel, "4USAGE:Mute <PlayerID> <TimeSecs> <Reason>");
		if (Mute[id] == 1) return IRC_GroupSay(groupID, channel, "4Player Is Already Muted");
		Mute[id] = 1;
  		format(str,sizeof(str),"You Have Muted For %d Secs By IRC Admin : %s [Reason: %s]",Time,user,reason);
	 	SCM(id,Green,str);
		format(str,sizeof(str),"%s(%d) Has Muted For %d Seconds By IRC Admin : %s [Reason: %s]",GetName(id),id,Time,user,reason);
		SCMTA(Green,str);
		SetTimer("mute",Time*1000,false);
		format(str,sizeof(str), "4,11Adminintstrator %s[%d] Has Muted %s[%d] (%d Seconds) | Reason : %s", user,GetName(id),id,Time,reason);
		IRC_GroupSay(groupID, IRC_CHANNEL, str);
		IRC_GroupSay(GAdmins,IRC_AdminChannel, str);
        return 1;
	}
